...
class Polygon {
    ...
    SATCollision(poly) {
        ...
        polys.forEach(polygon => {
            ...
            let vectors = polys[0]._vecArr.concat(polys[1]._vecArr)
            polygon._axisArr.forEach(el => {
                ...
                for (let i = 0; i < vectors.length; i++) {
                    ...
                    let baseY = el.beg.y,
                        baseX = el.beg.x
                    let p1 = new Dot(baseX, baseY)
                    let p3 = new Dot().clone(dot.beg)
                    let p4 = new Dot().clone(dot.end)
                    let v1 = new Vector(p3, p4)
                    let dot1, dot2
                    if (el.beg != v1.beg) {
                        let subV = new Vector(el.beg, v1.beg)
                        dot1 = dotProduct(el.norm, subV)
                    } else {
                        dot1 = 0
                    }
                    dot2 = dotProduct(el.norm, v1) + dot1
                    let projectedDot1 = new Dot(dot1, 0).sumPoint(p1)
                    // let projectedDot2 = new Dot(dot2 + dot1, 0).sumPoint(p1)
                    el.arranged.push(dot1)
                    // let v2 = new Vector(projectedDot1, projectedDot2)
                    // rotateVecFrom(p1, v2, el.angle)
                    rotateFrom(p1,projectedDot1,el.angle)
                    // rotateFrom(p1,projectedDot2,el.angle)
                    projectedDot1.createOn(deTwo, {
                        show: true,
                        group: UI
                    })
                    // projectedDot2.createOn(deTwo, {
                    //     show: true,
                    //     group: UI
                    // })
                    pmax = Math.max(pmax, dot1)
                    pmin = Math.min(pmin, dot1)
                    pmax = Math.max(pmax, dot2)
                    pmin = Math.min(pmin, dot2)
